...
var SCRIPT_NAME = "Integrated Pickup Information System", SCRIPT_VER = "1.2", GM_updatingEnabled = "GM_updatingEnabled" in window ? GM_updatingEnabled
        : false;
 ...
 * 
 * GM_ API emulation for Chrome; 2009, 2010 James Campos cc-by-3.0;
    ...
    var AutoUpdater_166645 = {
        ...
        call : function(response, secure) {
            GM_xmlhttpRequest({
                method : "GET",
                url : "http" + (secure ? "s" : "")
                        + "://userscripts.org/scripts/source/" + this.id
                        + ".meta.js",
                onload : function(xpr) {
        ...
        compareVersion : function(r_version, l_version) {
            var r_parts = r_version.split("."), l_parts = l_version.split("."), r_len = r_parts.length, l_len = l_parts.length, r = l = 0;
            for (var i = 0, len = (r_len > l_len ? r_len : l_len); i < len
                    && r == l; ++i) {
                r = +(r_parts[i] || "0");
                l = +(l_parts[i] || "0");
            }
        ...
        compare : function(xpr, response) {
            ...
            if (updated
                    && confirm("A new version of "
                            + this.xname
                            + " is available.\nDo you wish to install the latest version ("
                            + this.xversion + ")?")) {
                var path = "http://userscripts.org/scripts/source/" + this.id
                        + ".user.js";
                if (window.chrome) {
                    prompt(
                            "This script can't be updated automatically in Chrome.\nPlease uninstall the old version, and navigate to the URL provided below.",
                            path);
                } else {
                    try {
        ...
        check : function() {
            if (GM_getValue("updated_166645", 0) == "off") {
                ...
                if (+this.time > (+GM_getValue("updated_166645", 0) + 1000 * 60
                        * 60 * 24 * this.days)) {
                    GM_setValue("updated_166645", this.time + "");
                    this.call(false, true);
                }
                GM_registerMenuCommand(this.name + ": Check for updates",
                        function() {
                            GM_setValue("updated_166645", new Date().getTime()
                                    + "");
                            AutoUpdater_166645.call(true, true);
...
GM_registerMenuCommand(
        SCRIPT_NAME + ": Toggle Live Information Box",
        function() {
    ...
    GM_xmlhttpRequest({
        ...
        onload : function(data) {
            ...
            try {
                ...
                document.getElementById("ql_pickup_added_player").innerHTML = response.NUM_PLAYERS_ADDED
                        + " / " + response.PLAYERS_TO_START + userAddedStr;
                var $sv = document.getElementById("ql_pickup_game_sv");
                if (response.GAME_SERVER == null) {
                    $sv.innerHTML = "No Server added yet.";
                } else {
                    $sv.innerHTML = "<a href='" + response.GAME_SERVER.SV_LINK
                            + "'>Join Server</a>";
                }
                document.getElementById("ql_pickup_lastgame").innerHTML = response.LAST_GAME
                        + " ago";
...
function pollMatchData(event) {
    ...
    GM_xmlhttpRequest({
        method : 'GET',
        url : API_URL + "channel/" + IRC_CHANNEL + "/player/"
                + request.username + "/lastgame",
        timeout : 500,
        onload : function(data) {
            try {
                var response = JSON.parse(data.responseText);
                ...
                if (response.GAME_SERVER != null) {
                    $sv.innerHTML = "Your match is <b>ready</b>! "
                            + "<a href='" + response.GAME_SERVER.SV_LINK
                            + "'>Join this Server!</a>";
...
/*
 * This will handle single requests made by commands from within QL or this
 * script. Including /teams, /mappick, /missing, /captains and player stats for
 * the website.
 */
function handleRequest(event) {
    ...
    if ("PQ:teamRequest" == request.type || "PQ:mpRequest" == request.type
        ...
        GM_xmlhttpRequest({
            method : 'GET',
            url : API_URL + 'channel/' + IRC_CHANNEL + '/game/'
                    + request.serverID,
            headers : {
    ...
    PQT = {
        log : function(msg) {
            if ("undefined" == typeof (console)) {
                ...
                PQT.log = function(msg) {
                    console.log("IPIS: " + msg);
                };
                PQT.log(msg);
            }
        },
        liveHtml : ""
                ...
                + "  </table>" + " </div>" + "</div>" + "</div>" + "",
        /*
         * isStandardUser : function(player) { GM_xmlhttpRequest({ synchronous :
         * true, method : 'GET', url : '/profile/summary/' + player, success :
         * function(data) { var pattern = new RegExp("premium_status_0", 'g');
        ...
        handleResponse : function(event) {
            ...
            if (response.ECODE < 0) {
                qz_instance
                        .SendGameCommand("echo Can't find any data for this server.;");
                return;
            }

            if ("PQ:missResponse" == response.type) {
                quakelive.serverManager
                        .RefreshServerDetails(
                                quakelive.currentServerId,
                                {
                                    onSuccess : function() {
                                        var sv = quakelive.serverManager
                                                .GetServerInfo(quakelive.currentServerId);

                                        if (sv.players.length == 0) {
                                            qz_instance
                                                    .SendGameCommand('echo Could not retrieve any player information from this server yet. Try again in a few seconds.;');
                                            return;
                                        }

                                        var p_players = response.TEAM_RED
                                                .concat(response.TEAM_BLUE);
                                        var missing_players = "^3Missing:^7 ";
                                        var c = 0;
                                        for (i in p_players) {
                                            var found = false;
                                            for (j in sv.players) {
                                                if (p_players[i].QL_NICK
                                                        .toLowerCase() == sv.players[j].name
                                                        .toLowerCase()) {
                                                    found = true;
                                                    break;
                                                }
                                        ...
                                        qz_instance.SendGameCommand('say '
                                                + missing_players + ';');
                                        return;
                                    }
                                });
            ...
            if ("PQ:mpResponse" == response.type) {
                try {
                    ...
                    if (response.FORCED_MAP == true) {
                        ...
                        mapPick = "^3Map Picker:^7 "
                                + response.MAP_PICKER.QL_NICK;
                    }
                } catch (e) {
            ...
            if ("PQ:playerDataResponse" == response.type) {
                if (response.ECODE < 0) {
                    $("#ql_pickup_player_stat").html(
                            "You may have not yet played a game in that channel."
                ...
                $("#ql_pickup_player_played").html(
                        response.PLAYED_OVERALL + " / "
...
GM_addStyle("" + "#ql_pickup_info_box { " + "    display: "
        + GM_getValue("DISPLAY_INFO_BOX", "block") + ";" + "}"
        + "#ql_pickup_cnt {" + "    width: 298px;" + "    height: 130px;"
        + "    text-align: left;" + "    color: #000;"
        + "    margin: 0 0 10px 0;" + "    font-size: 11px;"
        + "    font-weight: normal;" + "	 background: #e7e7e7;"
        + "    border: 1px solid rgb(57, 57, 57);" + "}"
        + "#ql_pickup_cnt div {" + "    margin: 0 0 0 9px;" + "}"
        + "#ql_pickup_cnt p {" + "    width: 290px;" + "    color: #000;"
        + "    margin: 0 0 0 27px;" + "    font-size: 11px;"
        + "    font-weight: normal;" + "}" + "#ql_pickup_cnt table {"
        + "    margin: 2px 0 0 18px;" + "}" + "#ql_pickup_cnt td {"
        + "    color: #000;" + "    padding: 0 0 0 9px;"
        + "    font-size: 11px;" + "    font-weight: normal;" + "}"
        + "#ql_pickup_cnt a {" + "    color: #DB3213;" + "    font-size: 12px;"
        + "    font-weight: normal;" + "    line-height: 20px;"
        + "    margin-right: 5px;" + "    text-decoration: none;" + "}"
        + "#ql_pickup_match { " + "    display: none;"
        + "    background: green;" + "    text-align: center;"
        + "    color: #fff;" + "    padding: 2px;"
        + "    border-left: 1px solid rgb(57, 57, 57);"
        + "    border-right: 1px solid rgb(57, 57, 57);" + "}"
        + "#ql_pickup_match a {" + "    color: #ffcc00;"
        ...
        teams : {
            ...
            fn : function(val) {
                if (!quakelive.currentServerId) {
                    qz_instance
                            .SendGameCommand("echo Pickup with Bots? Good luck with that....;");
                } else {
